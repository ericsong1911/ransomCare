...
class MemoryDumpDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        segment = np.frombuffer(
            mem_dump[
                start_offset : min(start_offset + self.input_length, mem_dump_length)
            ],
            dtype=np.uint8,
        )
        ...
        key_offset = torch.tensor(
            (key_address - start_offset) / max(mem_dump_length, 1), dtype=torch.float32
        )
        # Key target output.
        key_tensor = torch.tensor(
            np.frombuffer(key, dtype=np.uint8), dtype=torch.float32
        )

        # Pad key to KEY_LENGTH
        padded_key = torch.zeros(self.key_length, dtype=torch.float32)
        padded_key[: len(key_tensor)] = key_tensor

        return segment, key_offset, padded_key, len(key)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, key_criterion, key_weight=0.5, location_weight=0.5):
    for epoch in range(num_epochs):
        model.train()
        train_loss = 0.0
        for inputs, location_targets, key_targets, key_lengths in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            if inputs is None:
                continue
            inputs = inputs.to(device)
            location_targets = location_targets.to(device)
            key_targets = key_targets.to(device)
            key_lengths = key_lengths.to(device)

            optimizer.zero_grad()
            location_outputs, key_outputs = model(inputs)
            location_loss = criterion(location_outputs.squeeze(), location_targets)

            # Calculate loss per key
            key_loss = 0
            for i in range(key_outputs.size(0)):
              key_loss += key_criterion(key_outputs[i, :key_lengths[i]], key_targets[i, :key_lengths[i]])
...
def evaluate_model(model, val_loader, criterion, device, key_criterion, key_weight=0.5, location_weight=0.5):
    ...
    val_loss = 0.0
    with torch.no_grad():
        for inputs, location_targets, key_targets, key_lengths in val_loader:
            if inputs is None:
                continue
            inputs = inputs.to(device)
            location_targets = location_targets.to(device)
